...
namespace MinhaBiblioteca
{
    ...
    { 0x50, 0x08, 0xF1, 0xDD, 0xDE, 0x3C, 0xF2, 0x18,
        ...
        public static string cryptoKey =
            "Aqsw34derf56wxcr34nuiydgUmluamRhZWwgLyBBRVM=";

        public static System.Security.Cryptography.RSA md = new RSACryptoServiceProvider(128);
        public static int QtdeConversoes = 2;
        private static int NumberConversoes = QtdeConversoes;
        ...
        private static string decriptografar(string hash)
        {
            try
            {
                return Criptografia.Decrypt(hash);
            }
            catch (Exception)
            { }
            return null;
        ...
        public static string criptografar(string texto, bool rsa)
        {
            NumberConversoes--;
            string result = null;
            try
            {
                result = Conversor.codificaBase64(texto);
            }
            catch (Exception)
            {
                //MessageBox.Show("Erro: " + ex.Message);
            }
            if (NumberConversoes > 0) return criptografar(result, rsa);
            NumberConversoes = QtdeConversoes;
            if (rsa) return criptografar(result);
            return result;
        }

        public static string decriptografar(string hash, bool rsa)
        {
            if (rsa) return decriptografar(decriptografar(hash), false);
            NumberConversoes--;
            string result = "";
            try
            {
                result = Conversor.decodificaBase64(hash).ToString();
            }
            catch (Exception)
            {
                //MessageBox.Show("Erro2: " + ex.Message);
            }
            if (NumberConversoes > 0) return decriptografar(result, rsa);
            NumberConversoes = QtdeConversoes;
            return result;
        }
    }
